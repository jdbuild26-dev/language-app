...
/**
 * Standardized speech matching with logging for debugging
 * @param {string} spokenText - The text transcribed from speech
 * @param {string} targetAnswer - The expected correct answer
 * @param {number} threshold - Similarity threshold (default 0.7)
 * @returns {boolean} - Whether the speech matches the answer
 */
export const matchSpeechToAnswer = (
  spokenText,
  targetAnswer,
  threshold = 0.7,
) => {
  if (!spokenText || !targetAnswer) return false;

  const isMatch = fuzzyIncludes(spokenText, targetAnswer, threshold);
  const score = calculateSimilarity(spokenText, targetAnswer);

  console.groupCollapsed(
    `🎤 Speech Match: "${spokenText}" vs "${targetAnswer}"`,
  );
