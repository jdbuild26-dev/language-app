...
import { Volume2, Table, TrendingUp, Map, Workflow } from "lucide-react";
...
const MOCK_QUESTIONS = [
    {
        ...
        instruction: "The table below gives information about the underground railway systems in six cities. Summarise the information by selecting and reporting the main features and make comparisons where relevant.",
        content: {
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50 dark:bg-slate-800/50">
                                            {currentQuestion?.content.headers.map((header, i) => (
                                                <th key={i} className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
                                                    {header}
                                                </th>
                                            ))}
                                        </tr>
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                        {currentQuestion?.content.rows.map((row, i) => (
                                            <tr key={i} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/5 transition-colors">
                                                {row.map((cell, j) => (
                                                    <td key={j} className="px-6 py-4 text-sm font-medium text-slate-700 dark:text-slate-300 truncate">
                                                        {cell}
                                                    </td>
                                                ))}
                                            </tr>
