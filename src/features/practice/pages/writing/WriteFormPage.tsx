...
import AudioPlayer from "@/features/vocabulary/components/shared/AudioPlayer";
...
export default function WriteFormPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await fetchPracticeQuestions("F7_Fill form");
      if (response && response.data && response.data.length > 0) {
        const transformed = response.data.map((q) => ({
          id: q.ExerciseID || Math.random(),
          instruction: q.Instruction_EN || "Fill in the blank",
          displaySentence:
  ...
  return (
    <>
      <PracticeGameLayout
        questionTypeFr="Remplissez le formulaire"
        questionTypeEn="Fill out the form"
