...
import { MessageSquare, User, Mic, Volume2 } from "lucide-react";
...
export default function SpeakingConversationPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await loadMockCSV("practice/speaking/speaking_conversation.csv");
  ...
  const handleToggleMic = () => {
    ...
    else { setSpokenText(""); recognitionRef.current.start(); setIsListening(true); }
  };

  const handleSubmit = useCallback(async () => {
    if (!currentExchange || !spokenText.trim() || isSubmitting || hasAnswered) return;
  ...
  }, [currentExchange, spokenText, isSubmitting, hasAnswered, isListening, evaluate, conversation, currentTurnIndex, totalExchanges]);
  ...
  return (
    <>
      <PracticeGameLayout
        ...
        isSubmitEnabled={spokenText.trim().length >= 2 && !hasAnswered && !!currentExchange}
                  ...
                  <div key={i} className="space-y-3">
                    {turn.speakerText && (
                      <div className="flex justify-start">
                        <div className="flex items-start gap-2 max-w-[85%]">
                          <div className="w-8 h-8 rounded-full bg-violet-100 dark:bg-violet-900 flex items-center justify-center flex-shrink-0 mt-1">
                    ...
                    {turn.userText && (
                      <div className="flex flex-col items-end gap-1">
                        <div className="bg-violet-500 text-white px-4 py-2 rounded-2xl rounded-br-md shadow-sm max-w-[85%]">
                        ...
                        {turn.evaluation && (
                          <div className={cn(
                            ...
                            <p className="text-sm mb-2 opacity-90">{turn.evaluation.executive_summary || turn.evaluation.feedback}</p>
                            ...
                            {turn.evaluation.pronunciation_tip && (
                              <p className="text-xs mt-2 opacity-70">🎙 {turn.evaluation.pronunciation_tip}</p>
                            )}
                          </div>
                        )}
                      </div>
              ...
              {currentExchange && !hasAnswered && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-4">
                  {currentExchange.prompt && (
                    <div className="w-full bg-violet-50 dark:bg-violet-900/20 rounded-2xl p-4 border border-violet-100 dark:border-violet-800/50">
                  ...
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-4">
                      {isListening ? "Tap again to stop" : "Tap the mic to start speaking"}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="text-xs text-slate-400">
                      {getWordCount(spokenText)} / {currentExchange.minWords || 3} words minimum
                    </div>
