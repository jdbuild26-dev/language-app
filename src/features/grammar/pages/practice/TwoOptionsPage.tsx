  ...
  useEffect(() => {
    const fetchAndTransformQuestions = async () => {
      try {
        ...
        const data = await loadMockCSV("grammar/two_options.csv", { tag });
        ...
        const transformed = ((data as any[]) || []).map((item) => {
          ...
          const uploadedOptions = uploadedOptionsFr.length
            ? uploadedOptionsFr
            : uploadedOptionsEn.length
              ? uploadedOptionsEn
              : uploadedOptionsGeneric;
          const options = uniqueOptions(parsedOptions.length ? parsedOptions : uploadedOptions, 2);
          const optionTranslations = uploadedOptions === uploadedOptionsFr ? uploadedOptionsEn.slice(0, 2) : [];
