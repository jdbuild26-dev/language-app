...
import { useStoriesByGrammar } from "../../../services/storiesApi";
  ...
  return (
    ...
    >
      ...
      <div className="relative mb-4">
        <div className="w-full h-32 bg-gradient-to-br from-green-100 to-teal-200 dark:from-green-900 dark:to-teal-800 rounded-xl flex items-center justify-center">
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className="flex-1 h-1.5 bg-teal-100 dark:bg-teal-900/30 rounded-full overflow-hidden">
          <div
...
export default function StoriesByGrammarPage() {
  const { topic } = useParams();
  const { stories, loading, error, getStoriesByGrammar } =
    useStoriesByGrammar();

  useEffect(() => {
    if (topic) {
      getStoriesByGrammar(topic);
    }
  }, [topic, getStoriesByGrammar]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">
          Stories: {displayTopic}
        </h1>
        <p className="text-gray-600 dark:text-slate-400 max-w-3xl mx-auto">
          Learn {displayTopic?.toLowerCase()} through engaging stories.
        </p>
      </div>
      ...
      {stories.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 dark:text-slate-400">
            No stories found for this grammar topic yet.
          </p>
        </div>
      )}
