...
import { LearningCard, LessonHeader } from "@/features/vocabulary/components/lesson-learn";
import {
  fetchReviewCards,
  removeFromReview,
...
export default function ReviewSessionPage() {
  ...
  const handleRemoveCard = async () => {
    if (!user || removingCard || cards.length === 0) return;
  ...
  const handleExit = () => {
    router.push("/vocabulary/lessons/review/words");
  };
      ...
      <div className="min-h-screen bg-white dark:bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 dark:text-slate-400 mb-4">
            No cards to review. Add some cards first!
          </p>
          <button
            onClick={handleExit}
            className="px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600 transition-colors"
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col">
      {/* Custom Header for Review Session */}
      <div className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors disabled:opacity-50"
          >
            {removingCard ? (
